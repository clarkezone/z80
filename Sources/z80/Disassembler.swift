...
public enum Disassembler {
    ...
    public static func replaceOperand(instruction: String, lowByte: UInt8, highByte: UInt8) -> String {
        if instruction.contains("**") {
            ...
            return instruction.replacingOccurrences(of: "*", with: byte)
        } else {
            return instruction
        }
    }
    ...
    private static func decodeInstruction(
        ...
        _ inst2: UInt8,
        _ inst3: UInt8,
        _ inst4: UInt8) -> String
    {
        let unknownOpcode = "<UNKNOWN>"

        switch inst1 {
            case 0xED: // extended instructions
                let opcode = inst1.toHex + inst2.toHex
                if !z80Opcodes.keys.contains(opcode) {
                    print("Opcode $opcode missing.")
                    return unknownOpcode
                } else {
                    return replaceOperand(instruction: z80Opcodes[opcode]!, lowByte: inst3, highByte: inst4)
                }
            ...
            case 0xDD, 0xFD:
                ...
                if inst2 == 0xCB // IX or IY bit instructions
                {
                    ...
                    let opcode = inst1.toHex + inst2.toHex + inst4.toHex
                    if !z80Opcodes.keys.contains(opcode) {
                        print("Opcode $opcode missing.")
                        return unknownOpcode
                    } else {
                        return replaceOperand(instruction: z80Opcodes[opcode]!, lowByte: inst3, highByte: 0)
                    }
                }
                if inst2 == 0x36 // LD (IX+*), *
                {
                    ...
                    if !z80Opcodes.keys.contains(opcode) {
                        ...
                        return replaceOperand(
                            instruction:
                            replaceOperand(instruction: z80Opcodes[opcode]!, lowByte: inst3, highByte: 0),
                            lowByte: inst4, highByte: 0)
                    }
                }
            ...
            default:
                ...
                let opcode = inst1.toHex
                if !z80Opcodes.keys.contains(opcode) {
                    print("Opcode $opcode missing.")
                    return unknownOpcode
                } else {
                    return replaceOperand(instruction: z80Opcodes[opcode]!, lowByte: inst2, highByte: inst3)
                }
        }
    }
        ...
        _ inst4: UInt8) -> Int
    {
        switch inst1 {
            ...
            case 0xDD, 0xFD: // IX instructions
                if inst2 == 0xCB {
    ...
    /// operands.
    public static func disassembleInstruction(_ byteSequence: [UInt8]) -> Instruction {
        ...
        let instruction = byteSequence + [0, 0, 0, 0]

        // This is the actual instruction length, based on opcode decoding
        let length = calculateInstructionLength(
            instruction[0], instruction[1], instruction[2], instruction[3])
        let disassembly = decodeInstruction(
            instruction[0], instruction[1], instruction[2], instruction[3])
        ...
        return Instruction(length: length, byteCode: byteCode.joined(separator: " "), disassembly: disassembly)
    }

    /// Takes an arbitrary length byte array and disassembles the first [count]
    /// instructions.
    public static func disassembleMultipleInstructions(
        instructions: [UInt8], count: Int, pc: UInt16) -> String
    {
        var result = ""
        var idx = 0
